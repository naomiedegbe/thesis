...
mod2 <- lm(auction ~ age + `number of bidders` + I(age^2) + I(`number of bidders`^2), data =auction)
summary(mod2)

mod3 <- lm(auction ~ age + `number of bidders` + I(`number of bidders`^2), data =auction)
summary(mod2)
```
...
```{r}
model2 <- lm(auction ~ age + `number of bidders` + age*`number of bidders`, data = auction)
summary(model2)
```
Second-order model
```{r}
model3 <- lm(auction ~ age + `number of bidders` + I(age^2) + I(`number of bidders`^2),
             data = auction)
summary(model3)
```
Second-order model + Interaction
```{r}
model4 <- lm(auction ~ age + `number of bidders` + I(age^2) + I(`number of bidders`^2) +
               age*`number of bidders`, data = auction)
summary(model4)
```
...
```
```{r model 5}
m5 <- lm(auction ~ age + `number of bidders` + factor(gender), data = auction)
summary(m5)
